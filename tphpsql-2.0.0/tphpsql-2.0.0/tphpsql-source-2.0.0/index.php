<?php
	...
	$required_opts_values = array('--config-file', '-d','--database','-s','--source-dir', '-l','--lib-dir', '--author', '--manager-suffix');

	//available options
	 $opts = array_merge($required_opts_values, array('--help','-h','--show-db', '--version', '-v', '--config'));

	//options to add in the files generated this variable is global used in the majority of the functions
	$options = array();

	/******************** default values *****************************/
	//database to use the global variable in some function (table_cols())
	$database = null;
			...
			if(in_array($val,$opts)){
				...
				if($val == '--config-file'){
					if(file_exists($value)){
						$content = parse_ini_file($value);
						if($content && is_array($content)){
							$options = array_merge($options, $content);
						}
						else{
							terror("invalid config ini file <br>");
						}
					}
					else{
						terror("config ini file '".$value."' does not exists<br>");
					}
				}
